...
1. Back on the desktop, right-click **DeviceComplianceLocalOnboardingScript.cmd**, then select **Run as administrator**. On the **User Account Control** dialog, select **Yes**.
...
1. On the **Name your DLP policy** page, enter the following values, then select **Next**:
...
1. In the new **Audit or restrict activities on devices** section:
    ...
    - Select the dropdown to the right of **Copy to a removable USB device** to change the action from **Audit only** to **Block**.
...
1. In the **Update cloud app mode** dialog, select **Yes** to activate the block mode.

1. Under **Service domains**, select **+ Add cloud service domain**.

1. On the **Add cloud service domain** flyout page, in the **Domain** field, enter `dropbox.com`, then select the **+** (plus) button to add the path. Select **Save** to save this setting.
